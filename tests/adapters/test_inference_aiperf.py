...
def _aiperf_runtime() -> ProviderRuntime:
...
def _aiperf_parser(tmp_path: Path, *, max_rows: int = 1_000_000) -> AIPerfRecordParser:
    return AIPerfRecordParser(
...
def _extractor(workspace: Workspace) -> InferenceArtifactExtractor:
    return InferenceArtifactExtractor(workspace, aiperf_runtime=_aiperf_runtime())


def _native_record(payload: dict[str, object]) -> dict[str, object]:
    result = dict(payload)
    raw_metadata = result["metadata"]
    assert isinstance(raw_metadata, Mapping)
    metadata = dict(raw_metadata)
    metadata.setdefault("request_end_ns", metadata["request_start_ns"])
    metadata.setdefault("worker_id", "worker-0")
    metadata.setdefault("record_processor_id", "processor-0")
    metadata.setdefault("benchmark_phase", "profiling")
...
def test_aiperf_record_parser_normalizes_safe_request_evidence(tmp_path: Path) -> None:
    ...
    rows = list(_aiperf_parser(tmp_path).iter_rows(result))
...
def test_aiperf_record_parser_keeps_only_safe_error_classification(tmp_path: Path) -> None:
    ...
    result.write_text(json.dumps(_native_record(payload)) + "\n", encoding="utf-8")
...
def test_aiperf_record_limit_counts_records_not_blank_lines(tmp_path: Path) -> None:
    ...
    payload = _native_record(payload)
    result.write_text("\n\n" + json.dumps(payload) + "\n" + json.dumps(payload) + "\n")
    parser = _aiperf_parser(tmp_path, max_rows=1)
...
def test_aiperf_rejects_unterminated_oversized_line(tmp_path: Path) -> None:
    ...
    with pytest.raises(DomainError) as error:
        list(_aiperf_parser(tmp_path).iter_rows(result))
...
def test_correlation_reports_all_matched_when_every_record_has_input(tmp_path: Path) -> None:
    ...
    parser = _aiperf_parser(tmp_path)
...
def test_extractor_correlates_with_inputs_json_and_surfaces_limitations(
    ...
    result = _extractor(workspace).extract_aiperf_result(imported.run.run_id)
...
def test_extractor_rejects_invalid_inputs_json_as_trace_artifact(tmp_path: Path) -> None:
    ...
    with pytest.raises(DomainError) as error:
        _extractor(workspace).extract_aiperf_result(imported.run.run_id)
...
def test_aiperf_extractor_publishes_prompt_free_request_evidence(tmp_path: Path) -> None:
    ...
    result_path.write_text(json.dumps(_native_record(payload)) + "\n")
    imported = ImportService(workspace).import_artifact(
        ImportArtifactRequest(
            path=result_path,
            kind=ArtifactKind.INFERENCE_RESULT,
