...
from flamo.domain import ArtifactKind, DomainError, ErrorCode, Sensitivity
from flamo.mcp import create_server, run_server
...
def workload_show(
    name: Annotated[str, typer.Argument(help="Named workload in flamo.toml.")],
    workspace: WorkspaceOption = None,
    json_output: JsonOption = False,
) -> None:
...
def experiment_plan(
    name: Annotated[str, typer.Argument(help="Named experiment in flamo.toml.")],
...
def extract_observations(
    run_id: Annotated[
        str,
        typer.Argument(help="Run containing Flamo SDK observations."),
    ],
    workspace: WorkspaceOption = None,
    json_output: JsonOption = False,
) -> None:
    """Extract bounded semantic annotations emitted by flamo.sdk."""
    try:
...
def mcp_serve(
    ...
    """Serve Flamo over stdio; stdout is reserved for protocol messages."""
