...
from flamo.adapters.builtins import BUILTIN_ADAPTERS, BuiltinAdapter, builtin_adapter
from flamo.adapters.registry import AdapterRegistry
from flamo.domain import CapabilityReport, CapabilityStatus, DomainError
            ...
            reports.append(
                CapabilityReport(
                    ...
                    remediation=(
                        ()
                        if package_version
                        else (f"Install Flamo's optional dependency for {adapter.name}.",)
                    ),
                )
            )
